...
- Create a `CheckDependency` model in `/app/hc/api/models.py` with the following fields:
    - `code`: UUID, auto-generated, unique, uneditable.
...
- `POST /api/v3/checks/<uuid:code>/dependencies/`: add a new dependency (write key required). Body field: `depends_on` (UUID of the check to depend on).
    - The `depends_on` check must belong to the same project.
    - A check cannot depend on itself.
    - Duplicate dependencies should.
    - Return error for a missing or invalid `depends_on` UUID.
    - On success return the created `CheckDependency.to_dict()`.
- `DELETE /api/v3/checks/<uuid:code>/dependencies/<uuid:dep_code>/`: remove a dependency by its `code` UUID (write key required). Returns respective result statuses (error and success).
...
- Return error if the check belongs to a different project than the API key. Return error if the check does not exist.
